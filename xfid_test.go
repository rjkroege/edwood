...
func TestXfidflush(t *testing.T) {
	...
	w1.body.file = MakeObservableEditableBuffer("", nil)
	w2 := NewWindow().initHeadless(nil)
	w2.body.file = MakeObservableEditableBuffer("", nil)
	row.col = []*Column{
		{
			w: []*Window{w1, w2},
		},
...
func TestXfidreadQWrdsel(t *testing.T) {
	...
	w := &Window{
		body: Text{fr: &MockFrame{}, file: MakeObservableEditableBuffer("", NewBuffer())},
		tag: Text{
			fr:   &MockFrame{},
			file: MakeObservableEditableBuffer("", NewBuffer()),
		},
		col: new(Column),
	}
...
func TestXfidwriteQWaddr(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			w.body.file = MakeObservableEditableBufferTag([]rune("abcαβξ\n"))
			w.col = new(Column)
...
func TestXfidwriteQWtag(t *testing.T) {
	...
	w := NewWindow().initHeadless(nil)
	w.col = new(Column)
	w.body.file = MakeObservableEditableBuffer("", nil)
	w.tag.file = MakeObservableEditableBuffer("", RuneArray(prevTag))
	x := &Xfid{
		fcall: plan9.Fcall{
			Data:  []byte(extra),
...
func TestXfidwriteQWerrors(t *testing.T) {
	...
	w := NewWindow().initHeadless(nil)
	w.col = new(Column)
	w.tag.file = MakeObservableEditableBufferTag(RuneArray("/home/gopher/edwood/row.go Del Snarf | Look "))
	w.tag.fr = &MockFrame{}
	w.body.fr = &MockFrame{}
	x := &Xfid{
		fcall: plan9.Fcall{
...
func TestXfidwriteQWeventExecuteSend(t *testing.T) {
	...
	w.tag = Text{
		w:       w,
		file:    MakeObservableEditableBuffer("", RuneArray("Send")),
		fr:      &MockFrame{},
		display: d,
	}
	w.tag.file.AddObserver(&w.tag)
...
func TestXfidreadQWbodyQWtag(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			switch tc.q {
			case QWbody:
				w.body.file = MakeObservableEditableBufferTag([]rune(data))
			case QWtag:
				w.tag.file = MakeObservableEditableBufferTag([]rune(data))
			}
...
func TestXfidruneread(t *testing.T) {
	...
	for _, tc := range tt {
		...
		w := NewWindow().initHeadless(nil)
		w.body.file = MakeObservableEditableBufferTag(tc.body)
...
func TestXfidreadQWxdataQWdata(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			w := NewWindow().initHeadless(nil)
			w.col = new(Column)
			w.body.file = MakeObservableEditableBufferTag([]rune(body))
			w.addr = tc.inAddr
			xfidread(&Xfid{
				f: &Fid{
...
func TestXfidreadQWctl(t *testing.T) {
	...
	w.body.fr = &MockFrame{}
	w.tag.file = MakeObservableEditableBufferTag([]rune(("/etc/hosts Del Snarf | Look Get ")))
	w.body.file = MakeObservableEditableBufferTag([]rune("Hello, world!\n"))
