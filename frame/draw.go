...
func (f *Frame) DrawSel(pt image.Point, p0, p1 int, highlighted bool) {
	...
	if f.ticked {
		log.Println("DrawSel hiding cursor", pt, p0, p1)
		f.Tick(f.Ptofchar(f.p0), false)
	}

	log.Println("if we haz selection, we should clear its...", f.p0, f.p1)
	if f.p0 != f.p1 {
		// Clear the selection so that subsequent code can
		// update correctly.
		back = f.Cols[ColBack]
		text = f.Cols[ColText]
		log.Println("clearing the selection", f.p0, f.p1)
...
//	i := 0
	...
	for ; nb < f.nbox && p < p1; nb++ {
		log.Println("Drawsel0 inner loop", "nb:", nb, "w:", w, p, string(f.box[nb].Ptr), f.p0, f.p1)
		b := f.box[nb]
		nr := b.Nrune
		...
		if p >= p0 {
			...
			f.cklinewrap(&pt, b)
//			log.Println("fill in the end of a wrapped line")
			if pt.Y > qt.Y {
...
//			log.Println("beginning...",  p0, p, runeindex(ptr, p0-p))
			ptr = ptr[runeindex(ptr, p0 -p):]
			nr -= int(p0) - p
//			log.Println("beginning of region...", p, p0, nr, string(ptr), string(b.Ptr[25:]))
			p = int(p0)
		}
...
//			log.Println("inside the Drawsel0", string(ptr[0:runeindex(ptr, nr)]), nr, w)
		}
		x = pt.X + w
		if x > f.Rect.Max.X {
			x = f.Rect.Max.X
		}
		log.Println("Drawsel0: drawing a highlight rect", image.Rect(pt.X, pt.Y, x, pt.Y+f.Font.DefaultHeight()), f.p0, f.p1)
		f.Background.Draw(image.Rect(pt.X, pt.Y, x, pt.Y+f.Font.DefaultHeight()), back, nil, pt)
		if b.Nrune >= 0 {
			// See comment above. Same issue applies.
//			log.Println("inside the Drawsel0 text drawing", string(ptr[0:runeindex(ptr, nr)]), pt)
...
func (f *Frame) Tick(pt image.Point, ticked bool) {
	log.Println("Tick", ticked, pt, f.p0, f.p1)
	if f.tickscale != f.Display.ScaleSize(1) {
		if f.ticked {
			f.tick(pt, false)
		}
		log.Println("Tick tickscaling",  f.p0, f.p1)
		f.InitTick()
	}
	
	f.tick(pt, ticked)
	log.Println("Tick end",  f.p0, f.p1)
}
