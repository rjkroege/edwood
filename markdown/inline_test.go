...
func TestInlineEquivParseInlineWithSourceMap(t *testing.T) {
	...
	// Note: empty input is tested separately because the unified function
	// intentionally normalizes to return 1 span (matching parseInlineFormatting),
	// while the existing parseInlineWithSourceMap returns 0 spans for empty input.
	tests := []struct {
		name string
		text string
	}{
		...
		{"unclosed_code", "`oops"},
		{"multiple", "a **b** *c* `d`"},
		{"link_with_bold", "[**b**](url)"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var gotEntries []SourceMapEntry
			var gotLinks []LinkEntry
			gotSpans := parseInline(tt.text, rich.DefaultStyle(), InlineOpts{
				SourceMap:      &gotEntries,
				LinkMap:        &gotLinks,
				SourceOffset:   0,
				RenderedOffset: 0,
			})
			wantSpans, wantEntries, wantLinks := parseInlineWithSourceMap(tt.text, rich.DefaultStyle(), 0, 0)
			compareSpans(t, gotSpans, wantSpans)
			compareSourceMapEntries(t, gotEntries, wantEntries)
			compareLinkEntries(t, gotLinks, wantLinks)
		})
	}
}

func TestInlineEquivParseInlineWithSourceMapNoLinks(t *testing.T) {
	// parseInline with SourceMap + NoLinks == parseInlineWithSourceMapNoLinks
	linkBase := rich.Style{
		Fg:    rich.LinkBlue,
		Link:  true,
		Scale: 1.0,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var gotEntries []SourceMapEntry
			gotSpans := parseInline(tt.text, linkBase, InlineOpts{
				NoLinks:        true,
				SourceMap:      &gotEntries,
				SourceOffset:   0,
				RenderedOffset: 0,
			})
			wantSpans, wantEntries, _ := parseInlineWithSourceMapNoLinks(tt.text, linkBase, 0, 0)
			compareSpans(t, gotSpans, wantSpans)
			compareSourceMapEntries(t, gotEntries, wantEntries)
		})
	}
...
func TestInlineEquivSourceMapWithOffset(t *testing.T) {
	// Verify offset handling matches existing function
	var gotEntries []SourceMapEntry
	var gotLinks []LinkEntry
	gotSpans := parseInline("**b**", rich.DefaultStyle(), InlineOpts{
		SourceMap:      &gotEntries,
		LinkMap:        &gotLinks,
		SourceOffset:   100,
		RenderedOffset: 50,
	})
	wantSpans, wantEntries, wantLinks := parseInlineWithSourceMap("**b**", rich.DefaultStyle(), 100, 50)
	compareSpans(t, gotSpans, wantSpans)
	compareSourceMapEntries(t, gotEntries, wantEntries)
	compareLinkEntries(t, gotLinks, wantLinks)
}
...
func compareSpans(t *testing.T, got, want []rich.Span) {
	t.Helper()
	if len(got) != len(want) {
	...
	for i := range got {
		if got[i].Text != want[i].Text {
			t.Errorf("span[%d].Text: got %q, want %q", i, got[i].Text, want[i].Text)
		}
		if got[i].Style != want[i].Style {
			t.Errorf("span[%d].Style: got %+v, want %+v", i, got[i].Style, want[i].Style)
		}
	}
}

func compareSourceMapEntries(t *testing.T, got, want []SourceMapEntry) {
	t.Helper()
	if len(got) != len(want) {
		t.Errorf("source map entry count: got %d, want %d", len(got), len(want))
		for i, e := range got {
			t.Logf("  got[%d]: %+v", i, e)
		}
		for i, e := range want {
	...
	for i := range got {
		if got[i].RenderedStart != want[i].RenderedStart ||
			got[i].RenderedEnd != want[i].RenderedEnd ||
			got[i].SourceStart != want[i].SourceStart ||
			got[i].SourceEnd != want[i].SourceEnd {
			t.Errorf("entry[%d]: got {RS:%d RE:%d SS:%d SE:%d}, want {RS:%d RE:%d SS:%d SE:%d}",
				i,
				got[i].RenderedStart, got[i].RenderedEnd, got[i].SourceStart, got[i].SourceEnd,
				want[i].RenderedStart, want[i].RenderedEnd, want[i].SourceStart, want[i].SourceEnd)
		}
	}
}

func compareLinkEntries(t *testing.T, got, want []LinkEntry) {
	t.Helper()
	...
	if len(got) != len(want) {
		t.Errorf("link entry count: got %d, want %d", len(got), len(want))
		return
	}
	for i := range got {
		if got[i] != want[i] {
